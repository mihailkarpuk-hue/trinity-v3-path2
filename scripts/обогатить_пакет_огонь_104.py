# -*- coding: utf-8 -*-
"""Обогатить огонь: params_104 на каждом атоме (копия клетки + пакет).

Эталон etalon_ogon.json не перезаписываем.
Запуск: python3 scripts/обогатить_пакет_огонь_104.py
"""
...
OUT_CELL = os.path.join(КОРЕНЬ, "выход", "атомы_полные_огонь", "etalon_ogon_with_104.json")
OUT_PKG = os.path.join(КОРЕНЬ, "выход", "атомы_полные_огонь", "атомы_звук_образ.json")
OUT_MD = os.path.join(КОРЕНЬ, "отчёты", "обогащение_огонь_params_104.md")
OUT_JSON = os.path.join(КОРЕНЬ, "отчёты", "обогащение_огонь_params_104.json")
...
def main() -> int:
    ...
    cell["обогащение_104"] = {
        ...
        "источник_звук": "данные/клеточки/эталоны/ogon_real.wav",
        "n_atoms": n_ok,
        "n_keys": n_keys,
